...
var (
	orderingOpts    = new(address_string_param.IPAddressStringParamsBuilder).AllowAll(true).SetRangeParams(address_string_param.WildcardAndRange).ParseEmptyStrAs(address_string_param.NoAddressOption).GetIPv6AddressParamsBuilder().AllowZone(false).GetParentBuilder().ToParams()
	macOrderingOpts = new(address_string_param.MACAddressStringParamsBuilder).AllowAll(true).AllowEmpty(true).SetRangeParams(address_string_param.WildcardAndRange).ToParams()
)
...
func (t addressOrderTest) testOrder() {
	...
	ipaddressStringOrderingSupplier := func(str string, i int) *Ordering {
		return &Ordering{
			nestedIPAddrString: goip.NewIPAddressStringParams(str, orderingOpts),
			order:              i,
		}
	}
	macaddressStringOrderingSupplier := func(str string, i int) *Ordering {
		return &Ordering{
			nestedMACAddrString: goip.NewMACAddressStringParams(str, macOrderingOpts),
			order:               i,
		}
	}
	...
	lowValComparator := goip.LowValueComparator
	...
	highValComparator := goip.HighValueComparator
