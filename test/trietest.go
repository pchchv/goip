...
func NewIPv4AddressAssociativeGenericTrie[V any]() *goip.AssociativeTrie[*goip.Address, V] {
	return &goip.AssociativeTrie[*goip.Address, V]{}
}
...
func (t trieTesterGeneric) run() {
	...
	for i, treeAddrs := range sampleIPAddressTries {
		_ = i
		addrs := collect(treeAddrs, func(addrStr string) *goip.Address {
		...
		addrsv6 := collect(treeAddrs, func(addrStr string) *goip.Address {
	...
	trie.Add(goip.NewIPAddressString("0.0.0.0/0").GetAddress().ToAddressBase())
	if trie.Size() != 1 {
		t.addFailure(newTrieFailure("unexpected size "+strconv.Itoa(trie.Size()), trie))
	}
	if trie.NodeSize() != 1 {
	...
	trie.Add(goip.NewIPAddressString("1.2.3.4").GetAddress().ToAddressBase())
...
func (t trieTesterGeneric) testString(strs trieStrings) {
	...
	assocTrie := &goip.AssociativeTrie[*goip.Address, int]{}
...
func countNodes(node goip.AssociativeAddedTreeNode[*goip.Address, int]) int {
	count := 1
...
func tcountNodes(node goip.AddedTreeNode[*goip.Address]) int {
	count := 1
	for _, n := range node.GetSubNodes() {
		count += tcountNodes(n)
	}
...
func (t trieTesterGeneric) testConvertedBlock(str string, expectedPrefLen goip.PrefixLen) {
	addr := t.createAddress(str).GetAddress()
	t.testConvertedAddrBlock(addr.ToAddressBase(), expectedPrefLen)
}

func (t trieTesterGeneric) testConvertedAddrBlock(addr *goip.Address, expectedPrefLen goip.PrefixLen) {
...
func (t trieTesterGeneric) partitionForTrie(trie *AddressTrie, subnet *goip.IPAddress) {
	goip.PartitionWithSingleBlockSize(subnet).PredicateForEach(IPAddressPredicateAdapter{trie.Add}.IPPredicate)
	if trie.Size() != 15 {
		t.addFailure(newTrieFailure("partition size unexpected "+strconv.Itoa(trie.Size())+", expected 15", trie.Clone()))
	}

	partition := goip.PartitionWithSingleBlockSize(subnet.ToAddressBase())
	all := goip.ApplyForEach[*goip.Address, *AddressTrieNode](partition, trie.GetAddedNode)

	if len(all) != 15 {
		t.addFailure(newTrieFailure("map size unexpected "+strconv.Itoa(trie.Size())+", expected 15", trie))
	}
	partition = goip.PartitionWithSingleBlockSize(subnet.ToAddressBase())
	keyAll := goip.ApplyForEach[*goip.Address, struct{}](partition, func(*goip.Address) struct{} { return struct{}{} })
	if len(keyAll) != 15 {
		t.addFailure(newTrieFailure("map size unexpected "+strconv.Itoa(trie.Size())+", expected 15", trie))
	}
	keyAll1 := make(map[goip.Key[*goip.Address]]struct{})
	for k, v := range keyAll {
		keyAll1[k.ToAddress().ToKey()] = v
	}

	all2 := make(goip.MappedPartition[*goip.Address, *AddressTrieNode])
	keyAll2 := make(map[goip.Key[*goip.Address]]struct{})
	goip.PartitionWithSingleBlockSize(subnet).ForEach(func(addr *goip.IPAddress) {
	...
	trie.Clear()
	goip.PartitionWithSpanningBlocks(subnet).PredicateForEach(IPAddressPredicateAdapter{trie.Add}.IPPredicate)
	if trie.Size() != 4 {
	...
	allAreThere := goip.PartitionWithSingleBlockSize(subnet).PredicateForEach(IPAddressPredicateAdapter{trie.Contains}.IPPredicate)
	allAreThere2 := goip.PartitionWithSpanningBlocks(subnet).PredicateForEach(IPAddressPredicateAdapter{trie.Contains}.IPPredicate)
...
func (t trieTesterGeneric) testRemove(addrs []string) {
	...
	t.testRemoveAddrs(ipv6Tree, addrs, func(addrStr string) *goip.Address {
		return t.createAddress(addrStr).GetAddress().ToIPv6().ToAddressBase()
	})
	t.testRemoveAddrs(ipv4Tree, addrs, func(addrStr string) *goip.Address {
		return t.createAddress(addrStr).GetAddress().ToIPv4().ToAddressBase()
	})
...
func (t trieTesterGeneric) testRemoveMAC(addrs []string) {
	tree := NewAddressGenericTrie()
	t.testRemoveAddrs(tree, addrs, func(addrStr string) *goip.Address {
		return t.createMACAddress(addrStr).GetAddress().ToAddressBase()
	})
...
func (t trieTesterGeneric) testRemoveAddrs(tree *AddressTrie, addrs []string, converter func(addrStr string) *goip.Address) {
	count := 0
	var list []*goip.Address
...
func (t trieTesterGeneric) testRemoveAddrsConverted(tree *AddressTrie, count int, addrs []*goip.Address) {
	tree2 := tree.Clone()
	tree3 := tree2.Clone()
	tree4 := tree2.Clone()
	tree5 := tree4.Clone()
...
func (t trieTesterGeneric) createIPv6SampleTreeAddrs(tree *AddressTrie, addrs []*goip.IPAddress) {
	for _, addr := range addrs {
		if addr.IsIPv6() {
...
func (t trieTesterGeneric) createIPv4SampleTreeAddrs(tree *AddressTrie, addrs []*goip.IPAddress) {
	for _, addr := range addrs {
		if addr.IsIPv4() {
			addr = addr.ToSinglePrefixBlockOrAddress()
			if addr != nil {
...
func (t trieTesterGeneric) testIterationContainment(tree *AddressTrie) {
	t.testIterationContainmentTree(tree, func(trie *AddressTrie) goip.CachingTrieIterator[*AddressTrieNode] {
...
func (t trieTesterGeneric) testIterationContainmentTree(
	trie *goip.Trie[*goip.Address],
	iteratorFunc func(addressTrie *AddressTrie) goip.CachingTrieIterator[*AddressTrieNode],
	addedNodesOnly bool) {
	iterator := iteratorFunc(trie)
	for iterator.HasNext() {
		next := iterator.Next()
		nextAddr := next.GetKey()
		var parentPrefix goip.PrefixLen
		...
		if !skipCheck { //panic: interface conversion: tree.C is nil, not *goip.PrefixBitCount
			if cached == nil {
				if parentPrefix != nil {
			...
			} else if !cached.(goip.PrefixLen).Equal(parentPrefix) {
				t.addFailure(newTrieFailure("mismatched prefix for "+next.String()+", cached is "+fmt.Sprint(iterator.GetCached())+" and expected value is "+parentPrefix.String(), trie))
			}
		}
...
func (t trieTesterGeneric) testIterate(tree *AddressTrie) {
	...
	t.testIteratorRem(tree, func(trie *AddressTrie) goip.IteratorWithRemove[*AddressTrieNode] {
	...
	t.testIterator(tree, func(trie *AddressTrie) goip.Iterator[*AddressTrieNode] {
...
func (t trieTesterGeneric) testIteratorRem(
	trie *AddressTrie,
	iteratorFunc func(*AddressTrie) goip.IteratorWithRemove[*AddressTrieNode],
...
func (t trieTesterGeneric) testIterator(
	trie *AddressTrie,
	iteratorFunc func(*AddressTrie) goip.Iterator[*AddressTrieNode],
	countFunc func(*AddressTrie) int) {
	// iterate the tree, confirm the size by counting
	// clone the trie, iterate again, but remove each time, confirm the size
	// confirm trie is empty at the end
...
func (t trieTesterGeneric) testContains(trie *AddressTrie) {
	...
	for iterator.HasNext() {
		...
		parent := next.GetParent()
		var parentPrefLen goip.PrefixLen
		if parent != nil {
			parentKey := parent.GetKey()
			parentPrefLen = parentKey.GetPrefixLen()
		} else {
			parentPrefLen = goip.ToPrefixLen(0)
		}
		prefLen := nextAddr.GetPrefixLen()
		var halfwayAddr *goip.Address
		var halfway goip.BitCount
		if prefLen == nil {
			prefLen = goip.ToPrefixLen(nextAddr.GetBitCount())
		}
		...
		if addedParent == nil {
			...
			var lastContaining *goip.ContainmentPathNode[*goip.Address]
...
func (t trieTesterGeneric) testEdges(trie *AddressTrie, addrs []*goip.Address) {
	trie2 := trie.Clone()
	for _, addr := range addrs {
		trie.Add(addr)
	}
...
func (t trieTesterGeneric) testAdd(trie *AddressTrie, addrs []*goip.Address) {
	trie2 := trie.Clone()
	trie3 := trie.Clone()
	trie4 := trie.Clone()
	k := 0
...
func (t trieTesterGeneric) testMap(trie *goip.AssociativeTrie[*goip.Address, any], addrs []*goip.Address,
	...
	var firstNode *goip.AssociativeTrieNode[*goip.Address, any]
	func() {
		defer func() {
