...
// and adjusted with each add and Remove operation in the sub-tree.
...
// ShortestPrefixMatch returns the beginning of the Path of containing subnets,
...
// LongestPrefixMatch returns the end of the Path of containing subnets,
...
// LastNode returns the last (the highest valued)
// node in the sub-trie originating from this node.
...
// LastAddedNode returns the last (the highest valued)
// added node in the sub-trie originating from this node,
// or nil if there are no added entries in this trie or sub-trie.
...
// NodeIterator returns an iterator that iterates through the added nodes of the sub-trie with this node as the root,
// in forward or reverse trie order.
...
// AllNodeIterator returns an iterator that iterates through all the nodes of the sub-trie with this node as the root,
// in forward or reverse trie order.
...
//
// If the argument is not a single address nor prefix block, this method will panic.
// The [Partition] type can be used to convert the
// argument to single addresses and prefix blocks before calling this method.
...
//
// Use Contains to check for the existence of a given address in the trie,
// as well as GetNode to search for all nodes including
// those not-added but also auto-generated nodes for subnet blocks.
