...
// because the conversion results in a subnet segment that is not a sequential range of values.
func (addr *IPv4Address) ToMaxHost() (*IPv4Address, address_error.IncompatibleAddressError) {
...
// ToGenericKey produces a generic Key[*IPv4Address] that can be used with generic code working
// with [Address], [IPAddress], [IPv4Address], [IPv6Address] and [MACAddress].
...
func (addr *IPv4Address) ToGenericKey() Key[*IPv4Address] {
	...
	// With Key[*IPv4Address], by leaving the scheme zero,
	// the zero Key[*IPv4Address] matches up with the key produced here by the zero address.
...
// CompareSize compares the counts of two subnets or addresses or other items,
// the number of individual addresses or items within.
//
// Rather than calculating counts with GetCount,
// there can be more efficient ways of determining whether this subnet represents more individual addresses than another item.
//
...
// (possibly two values if a range that is not a prefixed block),
// the number of digits according to the bit count,
// with or without a preceding "0" prefix.
//
// If a subnet cannot be written as a single prefix block or a range of two values, an error is returned.
func (addr *IPv4Address) ToOctalString(with0Prefix bool) (string, address_error.IncompatibleAddressError) {
	if addr == nil {
		return nilString(), nil
	}
...
//
// If a subnet cannot be written as a single prefix block or a range of two values, an error is returned.
func (addr *IPv4Address) ToBinaryString(with0bPrefix bool) (string, address_error.IncompatibleAddressError) {
	if addr == nil {
		return nilString(), nil
	}
