...
func (addr *IPv6Address) ToZeroHost() (*IPv6Address, address_error.IncompatibleAddressError) {
...
// a range in which all addresses have zero hosts,
// because the conversion results in a subnet segment that is not a sequential range of values.
func (addr *IPv6Address) ToZeroHostLen(prefixLength BitCount) (*IPv6Address, address_error.IncompatibleAddressError) {
...
func (addr *IPv6Address) toSinglePrefixBlockOrAddress() (*IPv6Address, address_error.IncompatibleAddressError) {
	if addr == nil {
...
func (addr *IPv6Address) ReverseBytes() (*IPv6Address, address_error.IncompatibleAddressError) {
...
func (addr *IPv6Address) ReverseBits(perByte bool) (*IPv6Address, address_error.IncompatibleAddressError) {
...
// ToGenericKey produces a generic Key[*IPv6Address] that can be used with generic code working with
...
func (addr *IPv6Address) ToGenericKey() Key[*IPv6Address] {
	// Note: We intentionally do not populate the "scheme" field,
	// which is used with Key[*Address] and Key[*IPAddress] and 64-bit Key[*MACAddress].
	// With Key[*IPv6Address], by leaving the scheme zero,
	// the zero Key[*IPv6Address] matches up with the key produced here by the zero address.
...
//
// If a subnet cannot be written as a single prefix block or a range of two values, an error is returned.
func (addr *IPv6Address) ToHexString(with0xPrefix bool) (string, address_error.IncompatibleAddressError) {
	if addr == nil {
		return nilString(), nil
	}
...
//
// If a subnet cannot be written as a single prefix block or a range of two values, an error is returned.
func (addr *IPv6Address) ToOctalString(with0Prefix bool) (string, address_error.IncompatibleAddressError) {
	if addr == nil {
		return nilString(), nil
	}
...
// NewIPv6AddressFromMACSection constructs an IPv6 address from a modified EUI-64 (Extended Unique Identifier)
// MAC address section and an IPv6 address section network prefix.
//
// If the supplied MAC address section is an 8-byte EUI-64, then it must match the required EUI-64 format of "xx-xx-ff-fe-xx-xx"
// with the "ff-fe" section in the middle.
//
...
func NewIPv6AddressFromMACSection(prefix *IPv6AddressSection, suffix *MACAddressSection) (*IPv6Address, address_error.AddressError) {
	return newIPv6AddressFromZonedMAC(prefix, suffix, NoZone)
}

// NewIPv6AddressFromZonedMACSection constructs an IPv6 address from a modified EUI-64 (Extended Unique Identifier) MAC address section,
// an IPv6 address section network prefix, and a zone.
//
...
func NewIPv6AddressFromZonedMACSection(prefix *IPv6AddressSection, suffix *MACAddressSection, zone string) (*IPv6Address, address_error.AddressError) {
	return newIPv6AddressFromZonedMAC(prefix, suffix, zone)
}

func newIPv6AddressFromZonedMAC(prefix *IPv6AddressSection, suffix *MACAddressSection, zone string) (*IPv6Address, address_error.AddressError) {
